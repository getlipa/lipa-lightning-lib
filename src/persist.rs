...
/// FilesystemPersister.
pub(crate) struct LipaPersister {
    callback: Arc<Box<dyn PersistCallback>>,
}

impl LipaPersister {
    /// Initialize a new FilesystemPersister and set the path to the individual channels'
    /// files.
    pub(crate) fn new(callback: Arc<Box<dyn PersistCallback>>) -> Self {
        Self { callback }
    }

    /// Read `ChannelMonitor`s from disk.
    pub(crate) fn read_channelmonitors<Signer: Sign, K: Deref>(
        &self,
        keys_manager: K,
    ...
    {
        let path = "monitors".to_string();
        if !self.callback.exists(path.clone()) {
            return Ok(Vec::new());
        }
        let mut res = Vec::new();

        for filename in self.callback.read_dir(path) {
            if filename.is_empty() || !filename.is_ascii() || filename.len() < 65 {
            ...
            let contents = self.callback.read("monitors/".to_string() + &*filename);
...
impl KVStorePersister for LipaPersister {
    fn persist<W: Writeable>(&self, key: &str, object: &W) -> std::io::Result<()> {
        let dest_file = key.to_string();
        let ser_object = object.encode();
        self.callback.write_to_file(dest_file, ser_object);
        Ok(())
    }
}
/*
...
mod tests {
    ...
    use crate::persist::FilesystemPersister;
