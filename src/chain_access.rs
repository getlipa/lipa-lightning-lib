...
mod tests {
    ...
    fn filter_is_initialised_empty() {
        ...
        let filter = LipaChainAccess::new(esplora_client.clone());
    ...
    fn tx_is_registered() {
        ...
        // Get initial length of filter vecs
        let initial_queued_tx_amount = filter.queued_txs.lock().unwrap().len();
        let initial_queued_output_amount = filter.queued_outputs.lock().unwrap().len();
        let initial_watched_tx_amount = filter.watched_txs.lock().unwrap().len();
        let initial_watched_output_amount = filter.watched_outputs.lock().unwrap().len();
        ...
        // Get final length of filter vecs
        let final_queued_tx_amount = filter.queued_txs.lock().unwrap().len();
        let final_queued_output_amount = filter.queued_outputs.lock().unwrap().len();
        let final_watched_tx_amount = filter.watched_txs.lock().unwrap().len();
        let final_watched_output_amount = filter.watched_outputs.lock().unwrap().len();

        assert_eq!(final_queued_tx_amount, initial_queued_tx_amount + 1);
        assert_eq!(final_watched_tx_amount, initial_watched_tx_amount);
        assert_eq!(final_queued_output_amount, initial_queued_output_amount);
        assert_eq!(final_watched_output_amount, initial_watched_output_amount);
    ...
    fn output_is_registered() {
        ...
        assert_eq!(final_queued_tx_amount, initial_queued_tx_amount);
        assert_eq!(final_watched_tx_amount, initial_watched_tx_amount);
        assert_eq!(final_queued_output_amount, initial_queued_output_amount + 1);
        assert_eq!(final_watched_output_amount, initial_watched_output_amount);
