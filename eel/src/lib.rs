...
impl LightningNode {
    ...
    pub fn pay_invoice(&self, invoice: String, metadata: String) -> Result<()> {
        ...
        if self
            ...
            .lock()
            .unwrap()
            .payment_exists(invoice_struct.payment_hash())?
        {
            return Err(runtime_error(
                RuntimeErrorCode::AlreadyUsedInvoice,
                "This invoice is already present in the local payment database. Either a previous payment attempt has been made or this invoice was generated by the local node. Please use a different one.",
            ));
        }
