...
fn test_decoding() {
    ...
    let result = node.util().decode_data(invalid_invoice);
    ...
    let data = node.util().decode_data(bitcoin_address.clone()).unwrap();
    ...
    let result = node.util().decode_data(node_id);
    assert!(matches!(
        result,
    ...
    let result = node.util().decode_data(url);
    assert!(matches!(
        result,
        Err(DecodeDataError::Unsupported {
    ...
    let data = node.util().decode_data(valid_invoice.clone()).unwrap();
    ...
    let result = node.util().decode_data(testnet_invoice);
    assert!(matches!(
        result,
        Err(DecodeDataError::Unsupported {
    ...
    let data = node.util().decode_data(lightning_address).unwrap();
    ...
    let data = node.util().decode_data(valid_lnurlp).unwrap();
    assert!(matches!(data, DecodedData::LnUrlPay { .. }));
    ...
    let data = node.util().decode_data(valid_lnurlw).unwrap();
